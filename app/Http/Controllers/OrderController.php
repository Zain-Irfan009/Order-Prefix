...
//dd(2);
        ...
        $log->log=json_encode($shop);
        $log->save();
        $setting=Setting::first();
        if($setting && $setting->update_prefix==1) {
            if ($order->financial_status != 'refunded' && $order->cancelled_at == null) {

                if ($order->company) {
                    $company_ids = [];
                    if ($setting->company_ids_excluded) {
                        $company_ids = explode(',', $setting->company_ids_excluded);
                    }
                    if (!in_array($order->company->id, $company_ids)) {
